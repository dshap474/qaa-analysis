"""
Temporal Features Extractor for QAA Analysis (Fixed Timezone Version).
...
class TemporalFeatures(FeatureExtractor):
    ...
    and transaction cadence analysis. All timestamps are handled as timezone-aware (UTC).
    """
    ...
    def _extract_basic_temporal_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Both timestamps should be timezone-aware now
        ...
        # Calculate active days (using UTC dates)
    ...
    def _extract_lifecycle_features(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Using timezone-aware DeFi start date
        defi_start_date = pd.Timestamp('2020-01-01', tz='UTC')
        ...
        if len(df_sorted) > 2:
            ...
            if len(daily_activity) > 1:
                ...
                n = len(x)
                if n * np.sum(x**2) - np.sum(x)**2 != 0:  # Avoid division by zero
